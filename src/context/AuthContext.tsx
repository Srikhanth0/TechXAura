...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    async function signIn(email: string, password: string) {
        try {
            ...
            // Reset rulesAccepted so the rules dialog shows every login
            const docRef = doc(db, "users", userCredential.user.uid);
            await setDoc(docRef, { rulesAccepted: false }, { merge: true });
            setProfile(prev => prev ? { ...prev, rulesAccepted: false } : null);
    ...
    async function signInWithGoogle() {
        try {
            ...
            if (!docSnap.exists()) {
                ...
                // Reset rulesAccepted so the rules dialog shows every Google login
                await setDoc(docRef, { rulesAccepted: false }, { merge: true });
                setProfile({ ...(docSnap.data() as UserProfile), rulesAccepted: false });
            }
