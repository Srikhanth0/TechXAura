...
export default function CheckoutPage() {
    ...
    const handleTeamSubmit = (members: TeamMember[], file?: File, url?: string) => {
        if (selectedEventId) {
            updateTeamMembers(selectedEventId, members, file, url);
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            // Using fetch to send data to Google Apps Script
            // Note: mode: 'no-cors' might be needed if script doesn't handle CORS correctly,
            // but standard Web App deployment usually handles it if configured as 'Anyone'.
            // However, 'no-cors' prevents reading response.
            // We assume standard setup from our guide works with CORS.
            const response = await fetch("/api/upload", {
                ...
                body: JSON.stringify({
                    ...
                    fileType: file.type, // Send as fileType if needed, or just type
                    name: `${user.uid}_${Date.now()}_payment.jpg`
                })
            });

            const data = await response.json();
            console.log("UPLOAD RESPONSE (payment):", data);

            if (!response.ok || !(data?.status === "success" || data?.success === true)) {
                throw new Error(data?.message || data?.error || "Upload failed");
            }

            const link = data.directLink || data.fileUrl || data.url || data.secure_url;
    ...
    const handleProceedToPayment = () => {
        ...
        // 2. Validation: Check for minimum team size
        for (const item of items) {
            ...
            if (item.event.id === "paperpresentation" && !item.abstractFile) {
                toast.error(`Event "${item.event.name}" requires an abstract file.`);
                return;
            }
        }
                ...
                {step === "payment" && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
                        ...
                        <div className="inline-block p-6 bg-white rounded-2xl mb-6">
                            <img src="/payment qr/payment.jpeg" alt="Payment QR Code" width={200} height={200} className="rounded-lg" />
                        </div>
...
function TeamMemberModal({
    ...
    existingFile?: File;
    existingUrl?: string;
    onSubmit: (members: TeamMember[], file?: File, url?: string) => void;
}) {
    ...
    }, [existingMembers, existingFile, existingUrl, open]);
    ...
    const handleSubmit = async () => {
        ...
        if (event.id === "paperpresentation" && abstractFile && !finalUrl) {
            setUploading(true);
            try {
                const scriptUrl = process.env.NEXT_PUBLIC_ABSTRACT_GOOGLE_SCRIPT_URL;
                ...
                const data = await response.json();
                console.log("UPLOAD RESPONSE (abstract):", data);
                ...
                finalUrl = data.fileUrl || data.url || data.secure_url || data.directLink;
                if (!finalUrl && data.data?.fileId) {
                    finalUrl = `https://drive.google.com/file/d/${data.data.fileId}/view`;
                }
                if (!finalUrl && data.data && typeof data.data === 'string') {
                    finalUrl = data.data;
                }
                ...
                toast.error("Failed to upload abstract. Please try again.");
                setUploading(false);
                return;
            }
            setUploading(false);
        ...
        onSubmit(members, abstractFile, finalUrl);
    };
