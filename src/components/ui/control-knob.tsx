...
export default function ReactorKnob() {
    ...
    const rawRotation = useMotionValue(-45);
        ...
        <div className="fixed inset-0 w-full h-full bg-neutral-950 flex flex-col items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 scale-125 md:scale-150">
                <div className="relative w-64 h-64 select-none">
                    ...
                    {/* PURPLE GLOW (Linked to Raw Input) */}
                    <motion.div
                        ...
                        >
                            ...
                            <div className="w-full h-full rounded-full bg-neutral-900 shadow-[0_10px_30px_rgba(0,0,0,0.8),inset_0_1px_1px_rgba(255,255,255,0.1)] border border-neutral-800 flex items-center justify-center relative overflow-hidden">
                                ...
                                <div className="relative w-24 h-24 rounded-full bg-neutral-950 shadow-[inset_0_2px_5px_rgba(0,0,0,1)] border border-neutral-800/50 flex items-center justify-center">
                                    ...
                                    <motion.div
                                        className="absolute top-3 w-1.5 h-5 bg-violet-500 rounded-full"
                                        style={{ boxShadow: useTransform(rawRotation, (r) => `0 0 ${Math.max(5, (r + 135) / 10)}px #8b5cf6`) }}
                                    />
...
function TickMark({ currentRotation, angle }: { currentRotation: MotionValue<number>, angle: number }) {
    ...
    // CHANGED: Orange to Violet/Purple
...
function DisplayValue({ value }: { value: MotionValue<number> }) {
    ...
    return (
        <div className="relative">
            {/* CHANGED: Text colors to Violet */}
            <span className="absolute inset-0 blur-sm text-violet-500/50 font-mono text-3xl font-black tabular-nums tracking-widest">
                {display.toString().padStart(3, '0')}
            </span>
            <span className="relative font-mono text-3xl text-violet-500 font-black tabular-nums tracking-widest">
                {display.toString().padStart(3, '0')}
