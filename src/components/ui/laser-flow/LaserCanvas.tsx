...
// Shader code from original energy-beam.tsx
const VERT = `
...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    // Manual derivatives approximation since R3F default implementation might differ
    float wL = pixW * 2.0; 
...
function LaserField({
    ...
    falloffStart = 1.2,
    fogFallSpeed = 0.6,
    color: propColor = '#00C2FF'
}: Props) {
    const materialRef = useRef<THREE.RawShaderMaterial>(null);
    const { gl, size, scene, camera } = useThree();

    // Pre-compile material to avoid runtime jank
    useEffect(() => {
    ...
    useEffect(() => {
        if (propColor !== '#00C2FF' && propColor !== '#BD36E2') {
            // If custom color passed, use it and don't oscillate
            setColor(propColor);
            return;
        }

        const interval = setInterval(() => {
            setColor(prev => prev === '#00C2FF' ? '#BD36E2' : '#00C2FF');
        }, 3000);
    ...
    const uniforms = useMemo(() => ({
        ...
        uFade: { value: 0 }
    }), []); // Dependencies intentionally empty, we update in useFrame or effects
    ...
    useEffect(() => {
        ...
        fogFallSpeed, color, size.width, size.height
    ]);
    ...
    useFrame((state) => {
        ...
        // Mouse (could be optimized to not query DOM every frame if needed, but R3F uses state.pointer)
        // Converting normalized pointer (-1 to 1) to shader mouse coords
        const px = (state.pointer.x + 1) / 2 * size.width;
        const py = (state.pointer.y + 1) / 2 * size.height;
        // Dampening could be added here similar to original implementation
        u.iMouse.value.set(px, py, 0, 0);
    });
...
export default function LaserCanvas(props: Props) {
    return (
        <Canvas
            frameloop="demand"
            dpr={[1, 1.5]}
