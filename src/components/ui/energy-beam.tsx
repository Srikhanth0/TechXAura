...
export const LaserFlow: React.FC<Props> = ({
    ...
    // Color oscillation: blue <-> purple every 3 seconds
    const [color, setColor] = React.useState(propColor || '#00C2FF'); // Blue

    useEffect(() => {
        if (propColor) return; // Don't oscillate if prop color is provided

        const interval = setInterval(() => {
            setColor(prev => prev === '#00C2FF' ? '#BD36E2' : '#00C2FF'); // Blue <-> Purple
        }, 3000); // 3 second delay
    ...
    useEffect(() => {
        ...
        const renderer = new THREE.WebGLRenderer({
            antialias: false,
            alpha: true, // Ensure this is false for opaque background if needed, or true if we want transparency
            depth: false,
            stencil: false,
